...
! get the tracer number for specific humidity
 ...
 if (do_init) then
    ...
    allocate(  e_global (is:ie, js:je, kdim-1))
    allocate(f_t_global (is:ie, js:je, kdim-1))
    allocate(f_q_global (is:ie, js:je, kdim-1))

    do_init = .false.

 endif

 call alloc_surf_diff_type ( Tri_surf, is,ie, js,je )
...
subroutine alloc_surf_diff_type ( Tri_surf, is,ie, js,je )

type(surf_diff_type), intent(inout) :: Tri_surf
integer,              intent(in)    :: is,ie,js,je

    allocate( Tri_surf%dtmass    (is:ie,js:je) )
    allocate( Tri_surf%dflux_t   (is:ie,js:je) )
    allocate( Tri_surf%dflux_q   (is:ie,js:je) )
    allocate( Tri_surf%delta_t   (is:ie,js:je) )
    allocate( Tri_surf%delta_q   (is:ie,js:je) )
