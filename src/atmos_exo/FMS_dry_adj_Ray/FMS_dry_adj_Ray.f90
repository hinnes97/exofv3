!! GKH-Lee - Sep 2020
! Module for dry convective adjustment following Ray's python notebook
!
!
!!
module FMS_dry_adj_Ray
  use, intrinsic :: iso_fortran_env
  implicit none
  !! Precision variables
  integer, parameter :: sp = REAL32
  integer, parameter :: dp = REAL64
  integer, parameter :: qp = REAL128

  integer, parameter :: itermax = 5
  real(kind=dp), parameter :: small = 1e-6_dp

contains

  subroutine Ray_dry_adj(nlay, nlay1, t_step, kappa, Tl, pl, pe, dT_conv)
    implicit none

    integer, intent(in) :: nlay, nlay1
    real(kind=dp), intent(in) :: kappa, t_step
    real(kind=dp), dimension(nlay), intent(in) :: Tl, pl
    real(kind=dp), dimension(nlay1), intent(in) :: pe
    real(kind=dp), dimension(nlay), intent(out) :: dT_conv

    integer :: i, iter
    logical :: did_adj

    real(kind=dp), dimension(nlay) :: Tl_cc, d_p
    real(kind=dp) :: pfact, Tbar

    Tl_cc = Tl

    do i = 1, nlay
      d_p(i) = pe(i+1) - pe(i)
    end do

    do iter = 1, itermax

      did_adj = .False.

      !! Downward pass
      do i = 1, nlay-1
        pfact = (pl(i)/pl(i+1))**kappa
        if (Tl_cc(i) < (Tl_cc(i+1)*pfact - small)) then
          Tbar = (d_p(i)*Tl_cc(i) + d_p(i+1)*Tl_cc(i+1)) &
             & / (d_p(i) + d_p(i+1))
          Tl_cc(i+1) = (d_p(i) + d_p(i+1))*Tbar &
             & / (d_p(i+1)+pfact*d_p(i))
          Tl_cc(i) = Tl_cc(i+1) * pfact
          did_adj = .True.
        end if
      end do

      !! Upward pass
      do i = nlay-1, 1, -1
        pfact = (pl(i)/pl(i+1))**kappa
        if (Tl_cc(i) < (Tl_cc(i+1)*pfact - small)) then
          Tbar = (d_p(i)*Tl_cc(i) + d_p(i+1)*Tl_cc(i+1)) &
             & / (d_p(i) + d_p(i+1))
          Tl_cc(i+1) = (d_p(i) + d_p(i+1))*Tbar &
             & / (d_p(i+1)+pfact*d_p(i))
          Tl_cc(i) = Tl_cc(i+1) * pfact
          did_adj = .True.
        end if
      end do

      ! If no adjustment required, exit the loop
      if (did_adj .eqv. .False.) then
        exit
      end if

    end do

    ! Change in temperature is Tl_cc - Tl
    ! adjust on timescale of 1 timestep
    dT_conv(:) = (Tl_cc(:) - Tl(:))/t_step

  end subroutine Ray_dry_adj

end module FMS_dry_adj_Ray
