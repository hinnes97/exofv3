...
! Cold start
       if ( Atm(n)%make_hybrid_z ) then
         ...
         if (grid_type < 4) then
            ...
            call init_case(Atm(n)%u,Atm(n)%v,Atm(n)%w,Atm(n)%pt,Atm(n)%tsurf,Atm(n)%delp,Atm(n)%q,Atm(n)%phis, Atm(n)%ps,Atm(n)%pe, &
         ...
         write(*,*) 'in fv_restart', minval(Atm(n)%delp(isd:ied,jsd:jed,1:npz))
