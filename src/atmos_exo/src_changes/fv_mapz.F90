module fv_mapz_mod
  ...
  use constants_mod,     only: radius, pi, rvgas, rdgas, grav, cp_vapor
  ...
  use interp_mod,        only: get_pk_edge, get_kappa_edge, get_pk_edge_2d
  implicit none
...
CONTAINS
 ...
 subroutine Lagrangian_to_Eulerian(do_consv, consv, ps, pe, delp, pkz, pk,   &
                      pk_temp, pdt, km, is,ie,js,je, isd,ied,jsd,jed,       &
                      ...
                      kap_loc, kap_edge, cp_loc, r_loc, non_dilute, master)
...
! !INPUT/OUTPUT
  ...
  real, intent(inout) :: pk_temp(is:ie,js:je,km+1)
...
! u-wind will be ghosted one latitude to the north upon exit
  ...
  logical, intent(in):: hybrid_z, master
  ...
  real,   intent(inout)  :: kap_edge(isd:ied,jsd:jed,km)
  real,    intent(inout) ::  cp_loc(isd:ied,jsd:jed,km)
  real,    intent(inout) :: r_loc(isd:ied,jsd:jed, km)
...
!
! For HII: Normal code usage is remap_t = .true.; CONVERT_T = .false.
! and te_map = .false.
! If te_map = .true. tries to remap from kinetic energy
! If remap_t = .false. remaps from virtual potential temperature
! apparently not recommended because increases exponentially near TOA
! If remap_t = .false. or te_map = .true. then pt variable used is a
! virtual potential temp. Otherwise it is virtual temperature
! For non-dilute core we turn back into a full thermo temperautre (non-virtual)
! If CONVERT_T = .true. then temperature is returned, if not the virtual potential
! temperature is returned (potential temp if non-dilute)
!-----------------------------------------------------------------------
  integer :: i,j,k 
...
! for nonhydrostatic option with hybrid_z coordinate
      ...
      real bkh, dtmp, dlnp(is:ie,js:je,km)
           ...
           if ( remap_t ) then
              if (non_dilute) then
                 do k=1,km
                    do j=js,je
                       do i=is,ie
                          !pt(i,j,k) = pt(i,j,k)* exp(kap_loc(i,j,k)* log(delp(i,j,k)/ (peln(i,k+1,j) - peln(i,k,j) )))/cp
                          !pt(i,j,k) = pt(i,j,k)*pkz(i,j,k)/cp_loc(i,j,k)
!pt(i,j,k) = pt(i,j,k) * exp(kap_loc(i,j,k)*log(delp(i,j,k)/(peln(i,k+1,j) - peln(i,k,j))))/cp_loc(i,j,k)
                          
                          !pt(i,j,k) = pt(i,j,k) * (pk_temp(i,j,k+1) - pk_temp(i,j,k))/(kap_loc(i,j,k)*cp_loc(i,j,k)*(peln(i,k+1,j) - peln(i,k,j)))
                          pt(i,j,k) = pt(i,j,k) * exp(kap_loc(i,j,k)*log(delp(i,j,k)/(peln(i,k+1,j) - peln(i,k,j))))
                       enddo
                    enddo
                 enddo
                 !if (master) write(*,*) 'mapz 1', maxval(pt(is:ie,js:je,1:km)),minval(pt(is:ie,js:je,1:km)), minval(pkz(is:ie,js:je,1:km)),maxval(pkz(is:ie,js:je,1:km))
              else
         ...
         te_map = .true.
           call pkez(km, is, ie, js, je, pe, pk, akap, peln, pkz, kap_loc(is:ie,js:je,:), non_dilute)
...
!           if (fill) call fillz(ie-is+1, km, 1, q2, dp2, q_source(is,j,iq))
       ...
       if (non_dilute) then
          ! Now that q has been regridded, recalculate the the value of R,cp,kappa
          vap = get_tracer_index(MODEL_ATMOS, 'vapour')
          do i=is,ie
                do k=1,km
                   r_loc(i,j,k) = rvgas*q(i,j,k,vap) + rdgas*(1. - q(i,j,k,vap))
                   cp_loc(i,j,k) = cp_vapor*q(i,j,k,vap) + cp*(1. - q(i,j,k,vap))
...
!---------------------
        ...
        if (non_dilute) then
           do k=km,1,-1
              do i=is,ie
                 !phis(i,k) = phis(i,k+1) + pt(i,j,k)*pkz(i,j,k)*akap*
                 !phis(i,k) = phis(i,k+1) + pt(i,j,k)*pkz(i,j,k)*kap_loc(i,j,k) * (peln(i,k+1,j) - peln(i,k,j))
                 !phis(i,k) = phis(i,k+1) + pt(i,j,k)*(pk3(i,k+1) - pk3(i,k))
                 phis(i,k) = phis(i,k+1) + pt(i,j,k)*pkz(i,j,k)*rg*(peln(i,k+1,j) - peln(i,k,j))
              enddo
           enddo
        else
           do k=km,1,-1
...
!------------
   if ( hydrostatic ) then
      if (non_dilute) then
         do k=1,km
            do i=is,ie
               !pkz(i,j,k) = (pk4(i,k+1) - pk4(i,k))/(kap_loc(i,j,k)*(peln(i,k+1,j) - peln(i,k,j)))
!pkz(i,j,k) = exp(kap_loc(i,j,k) * log((pe2(i,k+1) - pe2(i,k))/(peln(i,k+1,j) - peln(i,k,j)) ))
...
1000 continue
     ...
     !if (master) write(*,*) 'mapz 2', maxval(pt(is:ie,js:je,1:km)),minval(pt(is:ie,js:je,1:km)), minval(pkz(is:ie,js:je,1:km)),maxval(pkz(is:ie,js:je,1:km))
...
!$omp parallel do default(shared) private(i, j, k, gz, phis)
      do j=js,je
            ...
            if (non_dilute) then
               do i=is,ie
                  ...
                  do k=1,km
                     ...
                     !gz(i) = gz(i) + r_loc(i,j,k)*pt(i,j,k)*(peln(i,k+1,j) - peln(i,k,j))
                     gz(i) = gz(i) + rg*pt(i,j,k)*(peln(i,k+1,j) - peln(i,k,j))
                  enddo
               enddo
            else
         ...
         if (non_dilute) then
            do k=1,km
               do i=is,ie
                  te_2d(i,j) = te_2d(i,j) + delp(i,j,k)*(cp_loc(i,j,k)*pt(i,j,k)/(1. + r_vir*q(i,j,k,vap)) +   &
      ...
      else ! If remap_t = .false., pt is virtual potential temp
         if ( hydrostatic ) then
            if (non_dilute) then
               do i=is,ie
                  gz(i) = hs(i,j)
                  do k=1,km
                       !gz(i) = gz(i) + pt(i,j,k)*pkz(i,j,k)*akap*(peln(i,k+1,k)-peln(i,k,j))
                     !gz(i) = gz(i) + pt(i,j,k)*pkz(i,j,k)*kap_loc(i,j,k)*(peln(i,k+1,k)-peln(i,k,j))
                     !gz(i) = gz(i) + pt(i,j,k)*(pk_temp(i,j,k+1) - pk_temp(i,j,k))
                     gz(i) = gz(i) + pt(i,j,k)*rg*pkz(i,j,k)*(peln(i,k+1,j) - peln(i,k,j))
                    enddo
                 enddo
              else
                 do i=is,ie
            ...
            if (non_dilute) then
               do k=1,km
                  do i=is,ie
                     te_2d(i,j) = te_2d(i,j) + delp(i,j,k)*(cp_loc(i,j,k)*pt(i,j,k)*pkz(i,j,k)/(1.+r_vir*q(i,j,k,vap)) +   &
                          0.25*rsin2(i,j)*(u(i,j,k)**2+u(i,j+1,k)**2 +  &
                          v(i,j,k)**2+v(i+1,j,k)**2 -  &
                          (u(i,j,k)+u(i,j+1,k))*(v(i,j,k)+v(i+1,j,k))*cosa_s(i,j)))
                  enddo
...
!$omp parallel do default(shared) private(i, j, k)
! HII note I don't know what's going on here, so haven't changed
! for non-diluteness. Leave consv_te = 0.0!!
    if (non_dilute) then
       do j=js,je
          do i=is,ie
             zsum1(i,j) = cp_loc(i,j,k)*pkz(i,j,1)*delp(i,j,1)/(1. + r_vir*q(i,j,1,vap))
          enddo
          do k=2,km
             do i=is,ie
                zsum1(i,j) = cp_loc(i,j,k)*pkz(i,j,k)*delp(i,j,k)/(1. + r_vir*q(i,j,k,vap))
             enddo
          enddo
      ...
      if ( hydrostatic ) then
         if (non_dilute) then
            dtmp = tpe / (g_sum(zsum0,  is, ie, js, je, ng, area, 0))
         else
         ...
         do k=km,1,-1
            do i=is,ie
               tpe = te(i,j,k) - gz(i) - 0.25*rsin2(i,j)*(    &
                     u(i,j,k)**2+u(i,j+1,k)**2 + v(i,j,k)**2+v(i+1,j,k)**2 -  &
                     (u(i,j,k)+u(i,j+1,k))*(v(i,j,k)+v(i+1,j,k))*cosa_s(i,j) )
               ! Note actually R_d * dlnp
               dlnp(i,j,k) = rg*(peln(i,k+1,j) - peln(i,k,j))
            enddo
         enddo
      enddo
#ifdef CONVERT_T
         if (non_dilute) then
            ...
            do j=js,je
               do k=km,1,-1
                  do i=is,ie
                     !tmp = tpe / (cp_loc(i,j,k)*(1. - kap_loc(i,j,k)/rg * pe(i,k,j)*dlnp/delp(i,j,k)) )
                     tmp = tpe / (cp_loc(i,j,k) - pe(i,k,j)*dlnp/delp(i,j,k)*(1.+r_vir*q(i,j,k,vap)))
                     pt(i,j,k) =  tmp + dtmp*pkz(i,j,k)/(1. + r_vir*q(i,j,k,vap)) 
                     gz(i) = gz(i) + dlnp(i,j,k)*tmp*(1. + r_vir*q(i,j,k,vap))
                  enddo
               enddo
            enddo
         else
            do j=js,je
               do k=km,1,-1
                  do i=is,ie
                     tmp = tpe / ((cp - pe(i,k,j)*dlnp(i,j,k)/delp(i,j,k))*(1.+r_vir*q(i,j,k,sphum)))
                     pt(i,j,k) =  tmp + dtmp*pkz(i,j,k) / (1.+r_vir*q(i,j,k,sphum))
                     gz(i) = gz(i) + dlnp(i,j,k)*tmp*(1.+r_vir*q(i,j,k,sphum))
                  enddo
               enddo
            enddo
         endif
         ...
         if (non_dilute) then
            do j=js,je
               do k=km,1,-1
                  do i=is,ie
                     tmp = tpe / (cp_loc(i,j,k)/(1.+r_vir*q(i,j,k,vap)) - pe(i,k,j)*dlnp(i,j,k)/delp(i,j,k))
                     pt(i,j,k) = tmp/pkz(i,j,k) + dtmp
                     gz(i) = gz(i) + dlnp(i,j,k)*tmp
                  enddo
               enddo
            enddo
         else
            do j=js,je
               do k=km,1,-1
                  do i=is,ie
                     tmp = tpe / (cp - pe(i,k,j)*dlnp(i,j,k)/delp(i,j,k))
    ...
    if ( remap_t ) then
       ...
       if (non_dilute) then
          do k=1,km
             do j=js,je
                do i=is,ie
                   !pt(i,j,k) = (pt(i,j,k) + dtmp*pkz(i,j,k))/(1.+r_vir*q(i,j,k,vap))
                   pt(i,j,k) = (pt(i,j,k) + dtmp*pkz(i,j,k))/(1. + r_vir*q(i,j,k,vap))
                enddo
             enddo
          enddo
       ...
       if (non_dilute) then
          do k=1,km
             do j=js,je
                do i=is,ie
                   !pt(i,j,k) = cp_loc(i,j,k)*(pt(i,j,k)/pkz(i,j,k) + dtmp)
                   pt(i,j,k) = pt(i,j,k)/pkz(i,j,k) + dtmp
                   !pt(i ,j,k) = cp*(pt(i,j,k)/pkz(i,j,k) + dtmp)
                enddo
             enddo
          enddo
          !if (master) write(*,*)'mapz 3', maxval(pt(is:ie,js:je,1:km)),minval(pt(is:ie,js:je,1:km)),  minval(pkz(is:ie,js:je,1:km)), maxval(pkz(is:ie,js:je,1:km))
       else
          do k=1,km
             do j=js,je
                do i=is,ie
...
!     enddo
  ...
  subroutine pkez(km, ifirst, ilast, jfirst, jlast, &
                  pe, pk, akap, peln, pkz, kap_loc, non_dilute)
...
! Local
   ...
   real pk3(ifirst:ilast, km+1)
   real delp(ifirst-1:ilast+1,km)
   real pek
   real lnp
   real ak1
   integer i, j, k
...
!---- GFDL modification
       ...
       if (non_dilute) then
          do k=1,km
             do i=ifirst,ilast
                pkz(i,j,k) = exp(kap_loc(i,j,k)*log(delp(i,k)/(peln(i,k+1,j) - peln(i,k,j) )))
             enddo
          enddo
       else
          do k=1,km
