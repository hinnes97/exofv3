module fv_dynamics_mod
  use constants_mod,      only: grav, pi, radius, hlv, rdgas, rvgas, &
                                cp_vapor
   ...
   use fv_control_mod,     only: hord_mt, hord_vt, hord_tm, hord_tr, &
                                 ...
                                 inline_q, z_tracer, tau, rf_center, rf_lowest, nf_omega,   &
                                 remap_t,  k_top, p_ref, nwat, fv_debug, k_split, master, &
                                 non_dilute
...
!-----------------------------------------------------------------------
  ...
  subroutine fv_dynamics(npx, npy, npz, nq,  ng, bdt, consv_te, fill,               &
                        reproduce_sum, kappa, cp_a, zvir, ks, ncnst, n_split,     &
                        q_split, u, v, um, vm, w, delz, hydrostatic, pt, delp, q,           &
                        ps, pe, pk, pk_nd,peln, pkz, phis, omga, ua, va, uc, vc,          &
                        ak, bk, mfx, mfy, cx, cy, ze0, hybrid_z, time_total)

    real, intent(IN) :: bdt  ! Large time-step
    real, intent(IN) :: consv_te
    real, intent(IN) :: kappa, cp_a
    real, intent(IN) :: zvir
...
! Local Arrays
    ...
    ! HII Variable heat capacity
    real :: cp_local(isd:ied,jsd:jed,npz)
    real :: r_local(isd:ied,jsd:jed,npz)
    real :: kap_loc(isd:ied,jsd:jed,npz)
    real :: kap_edge(isd:ied,jsd:jed,npz+1)
    real :: pkz_temp(isd:ied,jsd:jed,npz)
    real :: pav(is:ie,js:je,npz)
...
!      real te_den
      ...
      !if (master) write(*,*) '1', maxval(pt(is:ie,js:je,1:npz)), minval(pt(is:ie,js:je,1:npz)), minval(pkz(is:ie,js:je,1:npz)), maxval(pkz(is:ie,js:je,1:npz))
!      ! HII added non-dilute effects here
      if (non_dilute) then
         ...
         do k=1,npz
            do j=js,je
               do i=is,ie
                  r_local(i,j,k)  = rvgas*q(i,j,k,vap) + rdgas*(1. - q(i,j,k,vap))
                  cp_local(i,j,k) = cp_vapor*q(i,j,k,vap) + cp_a*(1. - q(i,j,k,vap))
                  kap_loc(i,j,k) = r_local(i,j,k)/cp_local(i,j,k)
               end do
            end do
         end do
!         call get_pk_edge(npz, is, ie, js, je, peln(is:ie,:,js:je),kap_loc(is:ie,js:je,1:npz), pk_nd(is:ie,js:je,:), kap_edge(is:ie,js:je,1:npz+1))

         call mpp_update_domains(kap_loc, domain, complete=.true.)
 !        call mpp_update_domains(kap_edge, domain, complete=.true.)
         
!         if (master) then
!            write(*,*) '================================================================================================================='
!            write(*,*) 'Just before maxdiff 1'
!            write(*,*) maxval(kap_loc(is:ie,js:je,:)), maxval(kap_edge(is:ie,js:je,:))
!            write(*,*) maxval(pk_nd(is:ie,js:je,:))
!            write(*,*) maxval(pk(is:ie,js:je,:))
!            write(*,*) minval(kap_loc(is:ie,js:je,:))
!            write(*,*) minval(pk_nd(is:ie,js:je,:))
!            write(*,*) minval(pk(is:ie,js:je,:))
!
!            write(*,*) 'MAXDIFF', maxval(abs(kap_loc(is:ie,js:je,1:npz) - kappa)/kappa), maxval(abs((pk_nd(is:ie,js:je,:) - pk(is:ie,js:je,:))/pk(is:ie,js:je,:)))
!            write(*,*) maxval(pk_nd(is:ie,js:je, :)), minval(pk_nd(is:ie,js:je,:)), maxval(pk(is:ie,js:je,:)), minval(pk(is:ie,js:je,:))
!            in = maxloc(abs((pk_nd(is:ie,js:je,:) - pk(is:ie,js:je,:))/pk(is:ie,js:je,:)))
!            write(*,*) in
!            write(*,*) pk_nd(in(1), in(2), in(3)), pk(in(1), in(2), in(3))
!            write(*,*) peln(in(1), in(3), in(2)), kap_edge(in(1), in(2), in(3)), kap_loc(in(1),in(2),in(3))
!            write(*,*) exp(kap_edge(in(1), in(2), in(3))*peln(in(1),in(3),in(2)))
!
!            write(*,*) 'MAXMIN PKZ', maxval(pkz(is:ie,js:je,1:npz)), minval(pkz(is:ie,js:je,1:npz))
!         endif
!      
!        ! HI TEST
!         !kap_loc = kappa
!         !cp_local = cp_a
!         
!         ! Recalculate pressure^kappa
         do k=1,npz
            do j=js,je
               do i=is,ie
                  pkz(i,j,k) = exp(kap_loc(i,j,k) * log(delp(i,j,k)/(peln(i,k+1,j) - peln(i,k,j)) ))
!                  !pkz(i,j,k) = exp(rdgas/cp_a * (log(delp(i,j,k)/(peln(i,k+1,j) - peln(i,k,j)))))
!pkz_temp(i,j,k) = exp(kappa*log(pfull(k)))
                  !pkz(i,j,k) = (pk_nd(i,j,k+1)- pk_nd(i,j,k))/(kap_loc(i,j,k)*(peln(i,k+1,j) - peln(i,k,j)))
!                  !pkz(i,j,k) = (pk_temp(i,j,k+1) - pk_temp(i,j,k))/kap_loc(i,j,k)/(peln(i,k+1,j) - peln(i,k,j))
                  
               end do
            end do
         end do
!         if (master) then
!            write(*,*) 'max diff in pkz'
!            write(*,*) maxval(abs(pkz_temp(is:ie,js:je,1:npz) - pkz(is:ie,js:je,1:npz))/pkz(is:ie,js:je,1:npz))
!         endif
!         
      endif
...
!---------------------
      if ( consv_te > 0. ) then
           call compute_total_energy(is, ie, js, je, isd, ied, jsd, jed, npz,  &
                                     ...
                                     zvir, cp_a, rg, hlv, te_2d, ua, va, teq,  &
...
!              if(gid==0)  write(*,*) 'Total Energy Density (Giga J/m**2)=',te_den
      ...
      call Rayleigh_Friction(bdt, npx, npy, npz, ks, pfull, tau, rf_center, u, v, w, pt,  &
                             ua, va, delz, cp_a, rg,  hydrostatic, .true.)
...
! Convert pt to virtual potential temperature * CP
      ...
      if (non_dilute) then
         ...
         do k=1,npz
            do j=js,je
               do i=is,ie
! Defining potential temperature as cp * T * p^-kappa
! Hydrostatic equation becomes dphi = - kappa * p^kappa * pt * dlogp (see geopk in dyn_core)
! Any diabatic heating is from thermo equation:
! d(log(theta)) = Q/(cp*T)
! d (theta) = Q/p^kappa
! where Q is a heating per unit mass. Define pt like this because it is relatively
! simple to add heating from dissipation in way done currently
                  !cp_local(i,j,k) = q(i,j,k,vap)*cp_vapor + (1. - q(i,j,k,vap))*cp_a
!pt(i,j,k) = cp_local(i,j,k)*pt(i,j,k)/(pkz(i,j,k))
                  pt(i,j,k) = pt(i,j,k)*(1. + zvir*q(i,j,k,vap))/pkz(i,j,k)
               enddo
            enddo
         enddo
      else
         do k=1,npz
            do j=js,je
               do i=is,ie
                     pt(i,j,k) = cp_a*pt(i,j,k)/pkz(i,j,k)*(1.+zvir*q(i,j,k,sphum))
               enddo
            enddo
         enddo
      endif
      !if (master) write(*,*) '2', maxval(pt(is:ie,js:je,1:npz)), minval(pt(is:ie,js:je,1:npz)), minval(pkz(is:ie,js:je,1:npz)), maxval(pkz(is:ie,js:je,1:npz))
      !if (master) write(*,*) maxval(abs(zvir*q(is:ie,js:je,:,vap)))
#endif
...
!$omp parallel do default(shared) private(i, j, k)
      ...
      call dyn_core(npx, npy, npz, ng, vap, nq, mdt, n_split, zvir, cp_a, akap, grav, hydrostatic, &
                    u, v, um, vm, w, delz, pt, q, delp, pe, pk, pk_nd, phis, omga, ptop, pfull, ua, va,       & 
                    uc, vc, mfx, mfy, cx, cy, pem, pkz, peln, ak, bk, n_map==1, last_step,  &
                    kap_loc, kap_edge,time_total, ps, vap)

!      if (non_dilute) then
! Recalculate R, cp, kappa after dynamical core
!         do i=is,ie
!            do j=js,je
!               do k=1,npz
!                  R_local(i,j,k) = rdgas*(1.- q(i,j,k,vap)) + rvgas*q(i,j,k,vap)
!                  cp_local(i,j,k) = cp_a*(1. - q(i,j,k,vap)) + cp_vapor*q(i,j,k,vap)
!                  kap_loc(i,j,k) = R_local(i,j,k)/cp_local(i,j,k)
!
!                  if (pt(i,j,k) .ne. pt(i,j,k) )then
!                     write(*,*) 'Nan after dyn core!'
!                     stop
!                  endif
!                  
!               enddo
...
!      endif
      ...
      !if (master) write(*,*) '3', maxval(pt(is:ie,js:je,1:npz)), minval(pt(is:ie,js:je,1:npz)), minval(pkz(is:ie,js:je,1:npz)), maxval(pkz(is:ie,js:je,1:npz))
#ifdef SW_DYNAMICS
      do j=js,je
         do i=is,ie
...
!------------------------------------------------------------------------
         ...
         call Lagrangian_to_Eulerian(last_step, consv_te, ps, pe, delp,  &
                     pkz, pk, pk_nd,bdt, npz, is,ie,js,je, isd,ied,jsd,jed, &
                     nq, vap, u,  v, w, delz, pt, q, phis, zvir, cp_a,   &
                     ...
                     kap_loc, kap_edge, cp_local, r_local, non_dilute, master)

         call timing_off('Remapping')
         !if (master) write(*,*) '4', maxval(pt(is:ie,js:je,1:npz)), minval(pt(is:ie,js:je,1:npz)), minval(pkz(is:ie,js:je,1:npz)), maxval(pkz(is:ie,js:je,1:npz))
!--------------------------
...
!--------------------------
   ...
   if (non_dilute) then
      do k=1,npz
         do j=js,je
            do i=is,ie
               ! cp_local has already been recalculated by remapping algo
               !cp_local(i,j,k) = cp_vapor*q(i,j,k,vap) + cp_a*(1 - q(i,j,k,vap))
               !pt(i,j,k)  = pt(i,j,k)*pkz(i,j,k)/cp_local(i,j,k)!(cp_local(i,j,k))
!pt(i,j,k) = pt(i,j,k)*pkz(i,j,k)/(cp_a*(1.+zvir*q(i,j,k,vap)))
               pt(i,j,k) = pt(i,j,k) * pkz(i,j,k)/(1. + zvir*q(i,j,k,vap))
            enddo
         enddo
      enddo
      ...
      do k=1,npz
         do j=js,je
            do i=is,ie
               pt(i,j,k) = pt(i,j,k)*pkz(i,j,k)/(cp_a*(1.+zvir*q(i,j,k,sphum)))
            enddo
         enddo
      enddo
   endif
...
#endif
!if (master) write(*,*) '5', maxval(pt(is:ie,js:je,1:npz)), minval(pt(is:ie,js:je,1:npz)), minval(pkz(is:ie,js:je,1:npz)), maxval(pkz(is:ie,js:je,1:npz))
      deallocate ( dp1 )
      deallocate ( pem )
...
! local:
    ...
    if ( .not. RF_initialized ) then
          ...
          if( gid==0 ) write(6,*) 'Rayleigh friction E-folding time [days]: pressure (Pa), tau_drag (days))'
          c1 = 1. / (tau*sday)

          kmax = 1
          do k=1,npz
             if ( pm(k) < rf_lowest ) then
                  rf(k) = c1*(1.+tanh(log10(pc/pm(k))))
                  kmax = k
                  if( gid==0 ) write(6,*) k, 'pressure = ', pm(k),' Pa, ', 'tau = ', 1./(rf(k)*sday), ' days'
             else
                exit
             endif
          enddo
